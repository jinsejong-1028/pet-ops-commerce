...
서비스는 단일 운영사 B2C 커머스를 기준으로 설계합니다.

| 역할 | 주요 행동 |
|---|---|
| Guest | 상품 목록/상세 조회 |
| Member | 상품 구매, 장바구니, 주문, 결제 |
| Operator | 주문/재고/운영 이벤트 확인 |
| Admin | 상품 등록, 카테고리 관리, 쿠폰 관리 |

상품은 운영자가 등록한 서비스 상품 카탈로그입니다.
일반 사용자는 상품을 등록하거나 판매하지 않고 구매만 합니다.
...
- `product`는 운영자가 등록한 상품 카탈로그와 판매 상태를 담당한다.
